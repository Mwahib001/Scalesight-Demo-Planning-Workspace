...
A fictional, interactive demonstration of **Planning System + Ongoing Analysis + Recommendations + Strategic Guidance**. Built in the existing Next.js App Router project with TypeScript, Tailwind, Recharts, Lucide, and React Context. No backend or authentication. The strategy-call button opens an email to arman@scalesight.org.
...
All data and analyst-review identities are synthetic. No orders, ad changes, or messages are submitted by the planning demo. The strategy-call button opens the visitor’s email app with a message addressed to arman@scalesight.org; the visitor sends the email themselves. Existing `/forecast`, `/customer-growth`, `/partnership`, and `/advisor-brief` links redirect into the new workspace.
